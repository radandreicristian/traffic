...
class Conv2D(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        x = rearrange(
            x, "batch seq_len n_nodes n_features -> batch n_features n_nodes seq_len"
        )
...
class FullyConnected(nn.Module):
    ...
    def __init__(
        ...
        activations: Optional[
            Union[Callable[[Tensor], Tensor], List[Callable[[Tensor], Tensor]]]
        ],
        bn_decay: float,
        ...
        self.conv2ds = nn.ModuleList(
            [
                ...
                for in_features, out_features, activation in zip(
                    in_features, out_features, activations
                )
            ]
        )
...
class SpatioTemporalEmbedding(nn.Module):
    def __init__(self, d_hidden: int, bn_decay: float, use_bias: bool = True):
        super(SpatioTemporalEmbedding, self).__init__()
        self.fully_connected_spatial = FullyConnected(
            in_features=[d_hidden, d_hidden],
            out_features=[d_hidden, d_hidden],
            activations=[f.softplus, None],
            bn_decay=bn_decay,
            use_bias=use_bias,
        )
        self.fully_connected_temporal = FullyConnected(
            in_features=[31, d_hidden],
            out_features=[d_hidden, d_hidden],
            activations=[f.softplus, None],
            bn_decay=bn_decay,
            use_bias=use_bias,
    ...
    def forward(
        self, spatial_embeddings: Tensor, temporal_embeddings: Tensor
    ) -> Tensor:
        """
        ...
        spatial_embeddings = torch.unsqueeze(
            torch.unsqueeze(spatial_embeddings, dim=0), dim=0
        )
...
class SpatialAttention(nn.Module):
    def __init__(self, d_hidden, n_heads, bn_decay) -> None:
        ...
        self.fully_connected_q = FullyConnected(
            in_features=2 * d_hidden,
            out_features=d_hidden,
            activations=f.relu,
            bn_decay=bn_decay,
        )

        self.fully_connected_k = FullyConnected(
            in_features=2 * d_hidden,
            out_features=d_hidden,
            activations=f.relu,
            bn_decay=bn_decay,
        ...
        self.fully_connected_v = FullyConnected(
            in_features=2 * d_hidden,
            out_features=d_hidden,
            activations=f.relu,
            bn_decay=bn_decay,
    ...
    def forward(self, x, ste):
        ...
        keys = torch.cat(
            torch.split(self.fully_connected_k(x), self.d_head, dim=-1), dim=0
        )
        values = torch.cat(
            torch.split(self.fully_connected_v(x), self.d_head, dim=-1), dim=0
        )

        keys = rearrange(
            keys, "bxn_heads seq_len n_nodes d_head -> bxn_heads seq_len d_head n_nodes"
        )

        attention_scores = f.softmax(queries @ keys / self.d_head**0.5, dim=-1)
        attention = torch.cat(
            torch.split(attention_scores @ values, batch_size, dim=0), dim=-1
        )
...
class TemporalAttention(nn.Module):
    def __init__(
        self, d_hidden, n_heads, bn_decay, use_mask: bool = True, d_hidden_pos=None
    ) -> None:
        super(TemporalAttention, self).__init__()
        assert d_hidden % n_heads == 0, "Hidden size not divisible by number of heads."

        self.d_hidden_pos = d_hidden_pos if d_hidden_pos else d_hidden
        self.d_head = d_hidden // n_heads
        self.n_heads = n_heads
        self.use_mask = use_mask
        self.fully_connected_q = FullyConnected(
            in_features=d_hidden + self.d_hidden_pos,
            out_features=d_hidden,
    ...
    def forward(self, x, ste) -> Tensor:
        ...
        queries = rearrange(
            queries,
            "bxn_heads seq_len n_nodes d_head -> bxn_heads n_nodes seq_len d_head",
        )
        keys = rearrange(
            keys, "bxn_heads seq_len n_nodes d_head -> bxn_heads n_nodes d_head seq_len"
        )
        values = rearrange(
            values,
            "bxn_heads seq_len n_nodes d_head -> bxn_heads n_nodes seq_len d_head",
        ...
        attention_scores = queries @ keys / self.d_head**0.5
        if self.use_mask:
            mask = torch.tril(torch.ones(seq_len, seq_len).to(x.device))
            mask = repeat(
                mask, "a b -> j n a b", j=batch_size * self.n_heads, n=n_nodes
            ).to(torch.bool)
...
class GatedFusion(nn.Module):
    ...
    def forward(self, h_spatial, h_temporal) -> None:
...
class TransformAttention(nn.Module):
    def __init__(self, d_hidden, n_heads, bn_decay):
        super(TransformAttention, self).__init__()
        assert d_hidden % n_heads == 0, "Hidden size not divisible by number of heads."
        self.n_heads = n_heads
        self.d_head = d_hidden // n_heads
        self.fc_query = FullyConnected(
            in_features=d_hidden,
            out_features=d_hidden,
            activations=f.softplus,
            bn_decay=bn_decay,
    ...
    def forward(self, x, ste_previous, ste_future) -> torch.Tensor:
        batch_size, _, _, _ = x.shape
        queries = torch.cat(
            torch.split(self.fc_query(ste_future), self.n_heads, dim=-1), dim=0
        )
        keys = torch.cat(
            torch.split(self.fc_key(ste_previous), self.n_heads, dim=-1), dim=0
        )
        ...
        return self.fully_connected_out(
            torch.cat(torch.split(attention, batch_size, dim=0), dim=-1)
        )
